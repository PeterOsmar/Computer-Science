1\. A+  

&nbsp; &nbsp; **Language Comprehension**: High-level, interactive, interpreted array programming language designed for numerically intensive applications,

&nbsp; &nbsp; $~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~$ especially those found in financial applications

&nbsp; &nbsp; **Language Development History**

&nbsp; &nbsp; \- 1988

&nbsp; &nbsp; $~~$  Arthur Whitney began work on the A programming language to replace APL. Other developers at Morgan Stanley extended it to A+, adding a graphical user interface (GUI) and other language features.

&nbsp; &nbsp;  **Websites**: https://github.com/PlanetAPL/a-plus

2\. ABAP

&nbsp; &nbsp; **Full Name**: Advanced Business Application Programming

&nbsp; &nbsp; **Language Comprehension**: Domain-specific high-level programming language created by the German software company SAP SE

&nbsp; &nbsp; **Websites**: https://learning.sap.com/courses/basic-abap-programming

3\. ABC

&nbsp; &nbsp; **Language Comprehension**: Imperative general-purpose programming language and integrated development environment (IDE) developed

&nbsp; &nbsp; $~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~$ at Centrum Wiskunde \& Informatica (CWI), in Amsterdam, Netherlands by Leo Geurts, Lambert Meertens, and

&nbsp; &nbsp; $~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~$ Steven Pemberton

&nbsp; &nbsp; **Websites**: https://homepages.cwi.nl/\~steven/abc/programmers/introduction.html

